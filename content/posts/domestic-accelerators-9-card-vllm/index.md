...
- **A "free" MetaX card that wouldn't schedule.** The index-0 C500 showed 0% utilization and no processes — genuinely idle — yet every pod hit `NodeUnfitPod`. Root cause was a *driver-level* leak: `mx-smi` reported `total_vram_quota=3813` instead of the full `64549` MiB, because a previous sGPU tenant died without returning its quota. The fix was a four-layer teardown: warm-reset the card (`mx-smi -r -i 0 -y`), re-enable sGPU (quota snapped back to 64549), restart the device-plugin (its cache still said "native mode"), then restart the HAMi scheduler (it held a phantom allocation record). Only after all four did the slice schedule. **Lesson: on domestic sGPU stacks, "idle" is not "available" — you have to read the driver's quota, not the utilization.** That's a class of bug the NVIDIA ecosystem rarely hands you.
...
- **HAMi needs the resource-pool annotation** (`hami.io/resource-pool`) or the pod sits `Pending` with "resource pool default not found" — trivial once you know, an hour of confusion if you don't.
